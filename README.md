# Conformity, Trajectory, and Space: An Independently Re-Audited Analysis of Representational Survival in a Platform Service Market
...
3. **The reported 188/129 tourist-core split cannot be reproduced from the raw
   ...
   not the raw, unrepaired assignment taken at face value (342/58). See Figure 4,
   panel (b).
...
| **H3a/H3b** spatial reversal | β = −1.318\* (tourist) / β = −4.474† (non-tourist), reported as a clean sign reversal | β = −0.605, n.s. (tourist) / β = −1.137, n.s. (non-tourist) — **no significant effect in either subgroup**, no reversal | **Does not survive** the district repair |
| SAR spatial model (N=18 districts) | in-sample R² = 0.808 | not re-estimated here — **the panel does not contain 18 districts**, so the model as specified cannot be run on real data at all | Unresolved / not re-estimable as specified |
...
[`results/h3_split_comparison.csv`](results/h3_split_comparison.csv),
...
shape, and fill are used instead), serif-typeset, 300 dpi, generated by
`src/05_make_figures.py` from the CSVs in `results/`.
...
| `figures/fig2_h1_forest_plot.png` | H1a/H1b coefficients (M1–M3) with 90%/95% CI, forest-plot style. |
| `figures/fig3_h2_momentum_sensitivity.png` | Odds ratios (with 95% CI) for convergence and divergence momentum across 2–8 quarter accumulation windows, with N and event count k annotated at every point — the sensitivity sweep the earlier manuscript's single-window (4Q) reporting omitted. |
| `figures/fig4_h3_subgroup_and_split_sensitivity.png` | (a) H3 subgroup coefficients under the repaired district assignment — no significant effects. (b) The tourist-core/non-tourist split under three different district-handling methods, showing how unstable the earlier manuscript's 188/129 split is relative to anything reproducible from the raw file. |
...
│   ├── 04_h3_spatial_subgroup.py       # H3a/H3b: subgroup OLS + split-sensitivity check
│   └── 05_make_figures.py              # grayscale journal-style figures from results/*.csv
│
├── results/
│   ├── district_repair_diagnostics.csv
...
│   ├── h1_ols_results.csv
│   ├── h2_momentum_by_window.csv
│   ├── h3_subgroup_results.csv
│   └── h3_split_comparison.csv
│
├── figures/
│   ├── fig1_district_repair_diagnostic.png
│   ├── fig2_h1_forest_plot.png
│   ├── fig3_h2_momentum_sensitivity.png
│   └── fig4_h3_subgroup_and_split_sensitivity.png
│
└── docs/
...
`run_all.sh` runs `src/01` → `05` in order, printing each script's console diagnostics
...
2. **The SAR / Moran's I spatial model is not re-estimated in this repository.**
   ...
   specified**, not that it is confirmed at a smaller N.
...
repeated here as established findings.
